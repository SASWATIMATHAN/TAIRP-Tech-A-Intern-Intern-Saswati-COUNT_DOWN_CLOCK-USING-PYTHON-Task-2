...
window = tkinter.Tk()
window.title("Count-down timer by Saswati")
...
def set_alarm_command():
    bt.configure(text="Alarm is underway... ")
    ...
    while total_seconds_countdown > 0:
        ...
        if total_seconds_countdown == 0:
            bt.configure(text="Alarm chiming!!", fg="magenta")
...
tkinter.Label(window, text = "Alarm Clock", font=("Georgia", 50), fg = "teal", bg = "lavender").grid(row=0, column=0)

tkinter.Label(window, text = "(up to 24 hours)", font=("Arial Bold", 20), fg = "white", bg = "black").grid(row=1, column=0)

clock_button = tkinter.Button(window, text = "Tap for current time!", command = time_now)
clock_button.grid(row=2, column=0)

tkinter.Label(window, text = "Click on the button above to refresh current time.\n (Roughly-made clock for your reference)", fg = "teal").grid(row=3, column=0)

#H, M, S Labels
hours_label = tkinter.Label(window, text = "Hours")
hours_label.grid(row=1, column=1)
mins_label = tkinter.Label(window, text = "Minutes")
...
#Set Alarm button
bt = tkinter.Button(window, text="Configure Alarm", command = set_alarm_command)
